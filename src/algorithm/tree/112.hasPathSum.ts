...
;(() => {
    function hasPathSum(root: TreeNode | null, targetSum: number): boolean {
        ...
        let stack_node: TreeNode[] = [root],
            stack_val: number[] = [root.val]
        while (stack_node.length) {
            let node = stack_node.pop()!,
                val = stack_val.pop()!
            if (node.left === null && node.right === null) {
                if (val !== targetSum) continue
                return true
            }
            if (node.right) {
                stack_node.push(node.right)
                stack_val.push(node.right.val + val)
            }
            if (node.left) {
                stack_node.push(node.left)
                stack_val.push(node.left.val + val)
            }
        }
        return false
    }
    ...
    console.log(hasPathSum(trees, 22))
})()
